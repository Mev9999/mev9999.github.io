...
module.exports=function(root,source,out){
 ...
 for(const lang of ['de','en','bs']){
  ...
  const share=doc.createElement('button');share.type='button';share.className='form-share';share.dataset.share='';share.textContent={de:'Gewinnspiel teilen ↗',en:'Share giveaway ↗',bs:'Podijeli igru ↗'}[lang];top.append(share);
  ...
  for(const href of ['../scripts/accessibility-fixes.css','gewinnspiel-scoped.css']){const el=doc.createElement('link');el.rel='stylesheet';el.href=href;doc.head.append(el);}
