...
let now=Date.parse('2026-10-02T12:00:00+02:00'), rows=[],fail=false,requests=new Map();
...
Utilities:{getUuid:()=>`id-${rows.length+1}`},SpreadsheetApp:{flush(){},openById(){if(fail)throw Error('storage unavailable');return {getSheetByName:()=>({getLastRow:()=>rows.length+1,getRange:()=>({getValues:()=>rows,setValues(){}}),appendRow:r=>rows.push(r)})};}}};
vm.createContext(context);
vm.runInContext(code, context);
function submit(overrides={}){const p={name:'Test Person',email:'test.person@gmail.com',instagram:'test.person',eligibility:'on',terms:'on',parent_origin:'http://127.0.0.1:8771',request_id:'a'.repeat(32),text_version:'2026-09-25-v3',...overrides};const html=context.doPost({parameter:p}).html;return JSON.parse(html);}
...
assert.equal(submit({email:'different@example.org',instagram:'different.person'}).ok,true);assert.equal(rows.length,2);assert.equal(rows[1][10],'Namensgleichheit prüfen');
assert.equal(rows[0][12],'test.person');assert.equal(rows[0][13],'ungeprüft');assert.equal(rows[0][14],'ungeprüft');assert.equal(rows[0][15],'ungeprüft');
assert.equal(submit({instagram:''}).ok,false);assert.equal(submit({instagram:'https://instagram.com/test'}).ok,false);assert.equal(submit({instagram:'x'.repeat(31)}).ok,false);
...
now=Date.parse('2026-10-02T12:00:00+02:00');fail=true;assert.equal(submit({email:'new@example.org'}).ok,false);fail=false;
